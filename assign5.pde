...
void initClocks(){
  ...
  for (int i=0; i<clockX.length;i++){
    clockX[i] = SOIL_SIZE* floor(random(SOIL_COL_COUNT)); //0~80*8
    clockY[i] = SOIL_SIZE* (i*4+floor(random(4))); //0~80*24
...
void draw() {
	...
	switch (gameState) {
		...
		image(cabbage, cabbageX[i], cabbageY[i]);
			...
			if(isHit(cabbageX[i],cabbageY[i],playerX,playerY)
        && playerHealth < PLAYER_MAX_HEALTH) { // r1 bottom edge past r2 top
...
void drawTimerUI(){
	...
	String timeString = convertFramesToTimeString(gameTimer); // Requirement #4: Get the mm:ss string using String convertFramesTo(int frames)
  
  //seconds to mm:ss -> mm= secs/60 : ss= secs%60 
...
boolean isHit(float ax, float ay, float bx, float by){
  ...
  if( ax + SOIL_SIZE > bx    // r1 right edge past r2 left
        && ax < bx + SOIL_SIZE    // r1 left edge past r2 right
...
color getTimeTextColor(int frames){				// Requirement #5
  ...
  if(mins>=2){
  return #00ffff;
  }else if(mins<2&&mins>=1){
  return #ffffff;
  }else if(mins<1&&secs>=30){
  return #ffcc00;
  }else if(secs<30&&secs>10){
  return #ff6600;
  }else {
  return #ff0000;
  }
